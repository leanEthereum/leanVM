...
# Circulant Poseidon1-KoalaBear (width 16) MDS matrix. Stored as the first row;
# the full 16×16 matrix is `M[i][j] = MDS_CIRC_16_SHIFTS[(j - i) % 16]`.
MDS_CIRC_16_SHIFTS = (1, 1, 51, 1, 11, 17, 2, 1, 101, 63, 15, 2, 67, 22, 13, 3)

# Poseidon1-KoalaBear (width 16) constants used by the AIR. Tuples-of-tuples to stay on one line;
# raw u32s in canonical form. Synced with the Rust constants — `cargo test -p lean_prover --test
# check_poseidon1_constants` enforces a match. `initial_constants` and `final_constants` are the
# external (full) round constants generated by the Grain LFSR (Poseidon paper §5.3) with parameters
# field_type=1, α=3, n=31, t=16, R_F=8, R_P=20 — see
# https://github.com/Plonky3/Plonky3/blob/main/poseidon1/generate_constants.py. The `sparse_*`
# entries are the Plonky3/Horizen partial-round optimization derived from the partial RCs and MDS.
...
def _p1c() -> dict:
    ...
    n = len(MDS_CIRC_16_SHIFTS)
    mds_dense = [[Fp(MDS_CIRC_16_SHIFTS[(j - i) % n]) for j in range(n)] for i in range(n)]
    return {
