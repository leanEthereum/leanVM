...
fn compute_signer(index: usize) -> (XmssPublicKey, XmssSignature) {
    ...
    // `sequential = true`: this runs inside the parallel per-signer cache builder, so the
    // inner key-gen loops must not dispatch again (nested pool dispatch is forbidden).
