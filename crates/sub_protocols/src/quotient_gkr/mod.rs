...
mod tests {
    ...
    fn run_gkr_quotient(log_n: usize, active_chunks_frac: (usize, usize)) {
        ...
        let numerators_nat = MleOwned::BasePacked(pack_extension_in(&numerators_raw));
        let denominators_nat = MleOwned::ExtensionPacked(pack_extension_in(&denominators_raw));
