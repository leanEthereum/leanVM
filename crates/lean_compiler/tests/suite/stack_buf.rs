...
/// [`digest_cells`] for `sha3`: the digest cells of SHA3-256 of the 64 bytes
/// `a ‖ b`, what a `sha3(...)` output `StackBuf(2)` holds cell-by-cell.
fn sha3_digest_cells(a: [F64; 4], b: [F64; 4]) -> [F192; 2] {
    let bytes: Vec<u8> = a.into_iter().chain(b).flat_map(|w| w.0.to_le_bytes()).collect();
    hash_cells(&bytes)
}

/// The digest cells of the hash of `bytes`.
fn hash_cells(bytes: &[u8]) -> [F192; 2] {
    let d = primitives::keccak::hash(bytes);
...
";
    ...
    let want = sha3_digest_cells(h, h);

    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE).unwrap();
    assert_eq!(mix(src, want)[6], 1, "one SHA3 instruction");
...
def main():
    ...
    return
";
    let want = sha3_digest_cells([F64(1), F64(0), F64(2), F64(0)], [F64(3), F64(0), F64(4), F64(0)]);
    for flag in [0, 1] {
...
";
    ...
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE).unwrap();
    assert_eq!(mix(src, want)[6], 0, "no SHA3 here");
...
";
    ...
    let want = sha3_digest_cells([F64(7), F64(0), F64(5), F64(0)], [F64(2), F64(0), F64(3), F64(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE).unwrap();
    assert_eq!(mix(src, want)[6], 1, "one SHA3 instruction");
