...
pub(super) fn build(
    ...
    assert_eq!(
        rs_outputs.len(),
        rings.iter().map(|ring| ring.claims.len()).sum::<usize>()
    );
    ...
    // Each region's outputs, in the order the opener numbered them.
    let mut first = 0;
    let regions: Vec<_> = rings
        ...
        .map(|ring| {
            ...
            (ring.offset, ring.offset + (1 << ring.qflock_vars), outputs)
        })
        .collect();
