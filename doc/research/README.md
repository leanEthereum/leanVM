...
The subsequent [low-coordinate repair](body/flock/zk-flock-lowbank.tex) adds 3840 long-chain cycles and hides a complete sixteen-point coset in the low region jointly with the established VM/memory/root interface, below `2^-132`. Run `python3 doc/research/audits/zk_flock_lowbank_audit.py --full` for its valid read chains, exhaustive native query maps and exact error ledger. It assumes a common valid completion and still covers one selected coset, not the full opening.
