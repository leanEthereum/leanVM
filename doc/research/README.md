...
The current full-query candidate is ruled out for its fixed completion: [body/flock/zk-flock-query-fiber.tex](body/flock/zk-flock-query-fiber.tex) gives an explicit six-query test and simulator-error floors above `2^-93`, `2^-105`, `2^-114`, `2^-123` across the four rates. The default multicoset audit checks this certificate exactly. Its `--query-map --joint-boundary` mode diagnoses all initial lane-59 queries jointly with terminal metadata and the final GKR children; `--include-low` retains the public-prefix distribution, and `--source-validity` compares all raw-source polynomials with complete valid cycles. These sampled ranks are not a privacy bound. A repair must change the six-query observable before the full matrix criterion can meet the target.
