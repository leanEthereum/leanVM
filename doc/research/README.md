...
The subsequent [balanced long-chain repair](body/flock/zk-flock-lowbank.tex) uses 3840 cycles and hides a complete thirty-two-point high-region coset jointly with the established VM/memory/root interface, below `2^-151`. This joins two adjacent sixteen-point cosets using the same source. It also hides one sixteen-point coset throughout a larger region below `2^-132`. Run `python3 doc/research/audits/zk_flock_lowbank_audit.py --full` for valid read chains, exhaustive native query maps and exact error ledgers. It assumes a common valid completion and does not cover arbitrary nonadjacent cosets or the full opening.
