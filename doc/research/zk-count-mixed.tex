...
The script also checks binary rank $384$ for the full-support two-anchor root pair, the rational bound, and valid anchor placements outside the random support. The remaining task is a simulator or quantitative distribution bound for the complete mixed-bank view. The restricted root repair must not be composed with earlier marginal results by reusing the same randomness.
