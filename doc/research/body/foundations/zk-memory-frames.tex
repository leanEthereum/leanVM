...
\paragraph{Remaining common-sampler obligation.} Certify the address discipline and total allocation for the actual aggregation guest after wrapping, recheck its code reservation and static read cap, and replace ordinary filler by the common ZK cycle libraries. The candidate's free runs already exclude those libraries, so their cells cannot be charged twice. The public wrapper and allocation mechanism now exist, but a public resource theorem for every supported private execution and an integrated normalizing sampler do not. The all-layer GKR, remaining count/table/Flock, full PCS and authentication gaps are unchanged.
