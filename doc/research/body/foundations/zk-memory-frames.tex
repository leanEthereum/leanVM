...
\]
This relocation preserves each existing padding frame's internal relative offsets and count-chain lengths, but changes its address-dependent table coefficients. The capacity calculation alone does not certify all previous joint table-mask spans or a semantics-preserving transformation of the real program. The existing restricted Flock theorem must not silently be relabelled as a theorem for this variant.
