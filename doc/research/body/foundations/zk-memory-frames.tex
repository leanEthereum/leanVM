...
\paragraph{Conclusion.} Restricting programs to relocatable small frames is a useful repair, provided a public allocator reserves specific addresses for genuinely unused random words. Ordinary function calls alone do not provide this guarantee. We prove an obstruction for unrestricted execution-preserving padding, then a pin-preserving memory-value construction that avoids it and hides an entire restricted PCS algebraic view. Access counts, the joint bus transcript, the full stacked claim family and authentication remain outside the positive theorem.
...
\paragraph{Embedding in the actual stack.} If $\mu=\kmem+3$ and no column is larger than memory, the four memory-sized columns start at stack offset zero and each consists of eight of the actual sixty-four initial lanes. Each memory-value limb has the same reserved address pattern; an unused $\E$ word supplies independent masks in all three limbs. Within any one column the eight global fold weights share a selector on the other three lane bits. Excluding its zeros costs at most $3/|\E|$, so that column's restricted envelope has error at most $(\zkLaneLog+12)/|\E|$. Three such envelopes at one common column point cost at most $3(\zkLaneLog+12)/|\E|<2^{-185}$ under $\mu\le28$. This is a memory-value projection with the other stack data held fixed, not a theorem about secret-dependent count columns or the actual collection of PCS claims.
