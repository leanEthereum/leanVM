...
\paragraph{Result.} A redesigned local source jointly hides the twelve transmitted final GKR children, all $37$ BLAKE2s terminal claims, the established Flock endpoints and one selected lane-$59$ PCS query pair, below $2^{-155}$. The complete memory envelope and shared bus root join this view below $2^{-151}$. Its $26616$-row core still has an eight-query PCS obstruction, so four additional wider banks are reserved below. They preserve the proved bound and break that invariant. The resulting candidate uses $36856$ padding compressions and $62$ code locations, within unchanged public heights, replacing the previous two metadata libraries. Full PCS privacy and the preceding GKR messages remain open.
...
The extra $10240$ compressions and their returns use eight code locations. The enlarged source occupies $36856$ complementary compression positions, leaving $61448$ general-input positions, and uses $62$ code locations. Shared frames leave at most $181252$ Flock padding frames, or $5800064$ cells, and $26480$ remaining payload slots of size $256$. The audit's \texttt{--full --wide} option checks this larger allocation and all its read chains and randomizers. Breaking the invariant is not yet a quantitative simulator for eight query values. All pairs still stay within aligned blocks of sixteen; the earlier sixteen-query estimate alone does not rule out the target, but does not establish it either.

\paragraph{Next query interface.} On a fixed eight-point encoder coset inside $\gen^{18}+\subsp_{18}$, interpolation gives eight low-bit coefficient sums. For each child $t$, the core changes its two coefficients by the same scalar, while the wider bank changes their sum. This gives a concrete four-kernel/four-quotient route to a joint eight-value simulator. It requires the core span with one retained base-field coordinate in every child bank, and an exhaustive rank certificate for the wider quotient maps. These must be proved jointly with the current child/terminal interface before replacing the two-query theorem by an eight-query claim.
