...
\paragraph{Conclusion.} Two initial PCS queries defeat the original adjacent metadata swaps. Whole-row shuffling alone retains a second leak through equal count columns. Independent valid counter-label swaps before the shuffle repair the affected pair of lane-$59$ values below $2^{-160}$. The fixed-disclosure span theorem now composes that pair with all $37$ BLAKE2s terminal claims and the established Flock endpoints below $2^{-154}$, and with the memory envelope and shared bus root below $2^{-151}$. This is one selected pair in the specified coset, not the full query vector or its conditioning on omitted messages.
...
\paragraph{Remaining opening obligation.} This composition removes a specific shared-mask issue without extra rows, memory or verifier equations. It does not justify publishing another query pair, even in the same coset: those values impose further constraints on the same bits. The arbitrary fixed-disclosure lemma gives an explicit budget for this loss, not an unrestricted conditional oracle. The next target is the full joint linear observation map, including other lanes and the previous reductions. Common valid allocation and the missing GKR, table, Flock and authentication proofs remain separate obligations.

\paragraph{Evidence and literature boundary.} \auditref{zk_flock_pair_opening_audit.py} checks exact stack positions, coset interpolation, all displayed probabilities, disjoint library allocation and full counted-bus validity before and after the added randomizers. It also invokes the metadata audit to certify binary rank $3712$ for the nineteen terminal columns and raw PCS kernel coordinate together, and checks the full composition ledger exactly. Its \texttt{--native} option verifies the witness pairs and authenticated encoder identity; \texttt{--matching} runs the exhaustive certificate. Flock's unused witness slots are forced to zero, as specified and implemented in \path{crates/flock/src/hash.rs}, so they cannot serve as free masks. Bootle, Chiesa and Liu~\cite{bcl}, Section~2.4, add a separately designed R1CS gadget with controlled randomness and also rerandomize their sumcheck factors; this is not a theorem for arbitrary random compressions in leanVM's unchanged circuit and PCS.
