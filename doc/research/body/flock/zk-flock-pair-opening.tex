...
\paragraph{Conclusion.} The terminal theorem of \noteref{zk-flock-columns} is valid, but its adjacent-swap source is not sufficient for full ZK. Two initial PCS queries cancel its metadata randomness, giving simulator-error floors above $2^{-10},2^{-13},2^{-15},2^{-17}$ at the four supported rates. Adding a globally shuffled equal-compression library still fails: equality between its memory-count columns gives floors above $2^{-15},2^{-19},2^{-22},2^{-25}$. Independent valid counter-label swaps before the shuffle repair the affected pair of lane-$59$ values below $2^{-160}$, using an exhaustive finite-field certificate for every pair in the affected coset. The established terminal theorem survives. The full query vector and its conditional law remain open.
