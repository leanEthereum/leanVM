...
\paragraph{Conclusion.} Two initial PCS queries defeat the original adjacent metadata swaps, and whole-row shuffling alone retains a second leak. Independent valid counter-label swaps repair it. A shared-frame program bank then extends the joint theorem to all $37$ BLAKE2s terminal claims, the established Flock endpoints, one selected lane-$59$ query pair and the three actual GKR leaf evaluations, below $2^{-154}$. Adding the memory envelope and shared bus root remains below $2^{-151}$. This covers the computed leaf boundary, not the twelve transmitted GKR children or preceding rounds. The source changes only valid padding choices within the existing public heights.
