...
\paragraph{Conclusion.} Two initial PCS queries defeat the original adjacent metadata swaps, and whole-row shuffling alone retains a second leak. Independent valid counter-label swaps repair it. The current joint theorem includes all $37$ BLAKE2s terminal claims, the established Flock endpoints, one selected lane-$59$ query pair, and all ten BLAKE2s count evaluations at the GKR leaf point, below $2^{-154}$. Adding the memory envelope and shared bus root remains below $2^{-151}$. The count evaluations are an auxiliary interface, not the full GKR transcript. The larger theorem needs more random label choices in existing rows, with no height or verifier change.
...
\paragraph{Remaining opening and GKR obligations.} The auxiliary count vector now composes with the table claims and selected opening pair, not merely as a separate marginal. This does not simulate the full leaf triple, its preceding nonlinear GKR messages, the table sumcheck coefficients, additional query values or authentication. The next boundary step must include the address/value residual, and the final proof still needs a whole-layer theorem and one common valid sampler. The fixed-disclosure bounds are finite budgets, not permission to condition on an arbitrary transcript.
