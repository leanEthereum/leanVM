% !TeX root = ../../main.tex
\subsection{One inverse bank for the complete table sumcheck}\label{note:zk-table-batch}
...
\subsubsection{The polynomial the verifier actually checks}
...
\subsubsection{Inactive tables contribute a known linear term}
...
\[
 ...
 \in\E^{2n-2}
\]
is exactly the matrix proved onto in \noteref{zk-jump-rank}. Neither an extra masking polynomial nor a separate inverse bank per opcode is necessary for these coordinates.
...
\subsubsection{Last-round reconstruction}
...
\subsubsection{The exact remaining privacy target}
...
\paragraph{Progress versus the missing theorem.} The two-copy construction in \noteref{zk-jump-metadata} hides all JUMP non-inverse penultimate values jointly with the isolated constraint view. The subsequent \noteref{zk-jump-bus-padding} realizes a triangular counter layout: an upper-half bank masks $U$, then lower-half banks mask the four terminal counts, $H(t_*)$ and $T_J$ without changing $U$. Together with the other JUMP masks, it proves hiding of the bus-batched JUMP component with error below $2^{-146}$. It still does not condition on the preceding GKR messages. The other table targets, GKR, memory claims, Flock and PCS remain obligations; their marginal bounds cannot be composed into full ZK.

\paragraph{Executable evidence.} \auditref{zk_jump_bus_audit.py} checks the nonzero-target JUMP reconstruction using valid closed JUMP cycles and actual bus forms. \auditref{zk_table_batch_audit.py} tests all six tables at two unequal-height layouts, including height-zero tables. It derives bus forms from the actual layout placements, independently evaluates \eqref{eq:batch-poly} and interpolates its cubic messages, then compares reconstruction and replays both genuine and synthetic messages through the unchanged Python \texttt{table\_sumcheck}, without replacing its table list. Its other-table values are algebraic fixtures, not a bus-balanced VM execution or a Flock witness. The tests establish the reconstruction identities, not the missing joint hiding theorem.
