% !TeX root = ../../main.tex
\subsection{Composing JUMP padding with its sumcheck}\label{note:zk-jump-sumcheck}

\paragraph{Result and scope.} The terminal-only result in \noteref{zk-stacked-padding} does not directly extend to the sumcheck. This note identifies the missing last-round invariant and gives an explicit conditional simulator for both JUMP constraints together. The continuation \noteref{zk-jump-rank} now closes its quantitative rank hypothesis using a different, combined layout and proves statistical hiding of the complete isolated JUMP constraint sumcheck. Neither result covers the full table batch, bus or PCS.

\subsubsection{The invariant that inverse randomness cannot hide}
...
\subsubsection{An exact conditional simulation criterion}
...
\paragraph{Closing the criterion.} Sampled rank certificates are not a uniform probability bound for arbitrary real traces. A generic nonzero minor over base-field variables alone would give an error proportional to $2^{-64}$, which is insufficient. The combined construction in \noteref{zk-jump-rank} instead generates effective extension-field inverses, then proves a triangular minor nonzero independently of the real data. The smaller endpoint family below remains useful as a separate lemma. At least $3(2n-2)$ independent base-field inverse coins are necessary for this conditional strategy, before PCS observations are added.

\subsubsection{Valid cycles can supply the four endpoint masks}

The sparse set $S\subset\cube{12}$ from \noteref{zk-stacked-padding} has $448$ positions; its Boolean evaluation weights span $\E$ over $\Ftwo$ except with probability $\delta_{\mathrm{span}}+12/|\E|$, where $\delta_{\mathrm{span}}<2^{-158}$. In a $2^{15}$-row table, reserve, for each $a\in\{0,1\}$,
\[
\begin{array}{c|c}
