...
\begin{lemma}[An interval of exact transfers]\label{lem:zk-square-label-transfer}
...
\subsubsection{Reusing small frames in every normalization stage}

The preceding bytecode construction deliberately spends one fresh frame per traversal to leave all memory exponents unchanged. That is unnecessary when memory normalization follows it. Reuse the same frame throughout each of its eight instruction blocks instead, with distinct target operands and disjoint return-control cells. This changes the memory exponents in a controlled way and removes their linear frame-allocation cost. It does not reduce the number of instruction rows.

\begin{proposition}[Forty frames suffice for the bytecode stages]\label{prop:zk-reused-count-frames}
Let $\zkReuseBcInput{t}$ and $\zkReuseMemInput{t}{c}$ be the incoming bytecode and memory-column integer exponents. Apply the four noncompression, non-JUMP bytecode stages in disjoint eight-frame banks, then the JUMP stage in its own eight frames. Write $\zkReuseBcTarget{t}$ for the public bytecode exponent immediately after its normalization, including that stage's fixed four-square offset. Then every noncompression memory column has exponent
\[
 \zkReuseMemOutput{t}{c}
 =\zkReuseMemInput{t}{c}+\zkReuseBcTarget{t}-\zkReuseBcInput{t},
 \qquad t\in\{\mathrm{XOR},\mathrm{MUL},\mathrm{SET},\mathrm{DEREF},\mathrm{JUMP}\}.
\]
All compression rows and labels stay unchanged. Suppose their column products are already public. Under public incoming row totals, sufficient updated exponent bounds and disjoint row/code capacity, the later grouped routers, total-memory normalizer and power-of-two fillers still make all twenty-eight column products public. This entire noncompression completion needs only $62$ additional frames of at most $128$ cells, independently of its repetition counts.
\end{proposition}
\begin{proof}
An instruction repeated $n$ times in one fresh frame contributes $\binom n2$ to its bytecode exponent and to each distinct operand's memory exponent. Its closing JUMP contributes the same quantity in each of its own three disjoint cells. Thus a non-JUMP stage contributes $\zkReuseBcTarget{t}-\zkReuseBcInput{t}$ to every memory column of $t$ and to every JUMP memory column. All chains are complete and the fixed known operands satisfy every traversal. The JUMP stage sees the incoming bytecode exponent $\zkReuseBcInput{\mathrm{JUMP}}$ plus precisely these earlier closing-JUMP contributions. Its correction cancels that sum in each JUMP memory column, proving the displayed formula there too. The other columns are immediate.

Update the memory bounds using this exact affine transfer, then apply the total-memory and routing constructions above. No later variable stage changes a bytecode exponent. If the resulting total-memory interval has a nonzero lower endpoint, subtract that public endpoint before the four-square step and add it to the final public offset; it need not be nonnegative before this shift. The final fillers have public lengths, so each can also reuse one fresh frame: every additional exponent is then a publicly known triangular number. Fill JUMP last, after all their closing returns are counted. The frame count is $5\cdot8$ for bytecode, five grouped routers, eight memory-normalization frames, four preinstalled residue alternatives and five fillers. The existing templates use offsets below $128$, and every indirect target is local to its frame. They can be packed two per aligned $256$-cell slot, outside the prefix and masks. This proves the conditional construction, not that its row or code budget fits the current candidate.
\end{proof}

\paragraph{A complete cycle certificate.} The \texttt{--reuse-frames} mode of \auditref{zk_column_count_audit.py} checks sixteen executions with varying instruction multiplicities and memory reuse. The second choice scatters repeated instructions across two preinstalled frames without changing their code visits, so memory and bytecode uncertainty are genuinely different. Every completed library has row totals $(64,256,64,64,512,8)$, the same twenty-eight products and full memory/code values, and the same untouched compression rows. All ISA constraints, bus multisets and complete read chains pass. In this certificate the updated uncertain memory total is in $[1079,1388]$ before fixed router offsets. Center $11$ normalizes its width $309$; router size $14$ covers every remaining column deficit, at most $186<14^2$. Exactly $62$ frames are added. These bounds are deliberately conservative and belong to this finite family, not the aggregation guest.

The construction makes concrete use of small frames, but does not make their counts disappear. Memory final-count openings, the remaining coarse and finer count frontiers, changed filler disclosures and the full joint transcript still require proof. Its $31$ packed slots exclude the real computation, the existing BLAKE2s libraries and the initial completion to public opcode totals. Establish that common row/code ledger before interpreting the frame saving as candidate-wide feasibility.
