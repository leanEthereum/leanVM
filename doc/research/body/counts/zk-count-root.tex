% !TeX root = ../../main.tex
\subsection{A fixed-length cycle library that fixes the count root}\label{note:zk-count-root}
...
\subsubsection{The exponent and a public interval}
...
\subsubsection{The two valid cycle primitives}
...
\subsubsection{Exact normalization with fixed length}
...
\]
Thus the non-JUMP product is recoverable except with probability $1/|\E|$, even when $P_JP_{\mathrm{other}}$ is fixed publicly. Its privacy needs more than JUMP normalization. This is an obstruction to a JUMP-only GKR wrapper, not to valid padding distributed among the other tables. The distributed construction in \noteref{zk-count-columns} now fixes every individual count-column product, repairing this coarse leak. GKR observations inside the columns remain open; \noteref{zk-count-children} handles its final count-child packet alone with product-preserving counter trades.

\paragraph{Executable evidence.} \auditref{zk_count_root_audit.py} exhaustively checks the normalization arithmetic for deficits through $1024$, then builds sixteen actual ISA instances, including zero loop multiplicities and all four residues. Each has the same row count and memory image; the actual bus identities and product of transmitted count labels give the public root. It also checks the integer power-of-two scheduling formula. Its small four-square search is an audit helper, not a production algorithm or a claim that every example fits current PCS limits.

The separate \auditref{zk_count_gkr_audit.py} checks the actual block placements and replays two accepted layers of the reference GKR verifier for two different non-JUMP products with the same total root. It then recovers each product from the displayed child value. These intentionally incomplete algebraic prefixes certify the extraction formula; they are not full bus-balanced VM proofs.
