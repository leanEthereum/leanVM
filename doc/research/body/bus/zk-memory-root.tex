...
\paragraph{Result and scope.} The memory bank of \noteref{zk-memory-frames} also hides the shared bus product root jointly with its three-limb opening envelope, with combined error below $2^{-151}$. No extra reserved words are needed. The argument finds independent private directions in the kernel of that entire linear opening view, then mixes the product root along those directions. It is not a simulator for intermediate GKR products, table messages, access counts, the full stacked claim family or authentication. All challenges are independent honest interactive coins; the memory observation map must be independent of the bus fingerprint coins.

\subsubsection{Private directions left after opening}

Use the memory-dominant stack in \noteref{zk-memory-frames}, with eight initial segments per value limb and five fully random segments $\zkFreeLanes$. Write their restricted global fold weights as a common selector times $\alpha_l$, $l\in\zkFreeLanes$. Let $V$ now retain \emph{all} distinct initial query answers, including $0,1$, and one common column MLE, expanded into three $\K$ coordinates. It has at most $q+3$ rows over $\K$.
...
\paragraph{Boundary and evidence.} This proves the shared root jointly with a full restricted linear opening view, not arbitrary conditioning of the old marginal root theorem. The intermediate GKR messages are nonlinear in the selected parameters and generally destroy their kernel property; they still require a new proof. The same warning applies to the actual full stacked claim family and to hash commitments. A public normalized count root can be adjoined, but a secret-dependent count view cannot. \auditref{zk_memory_frames_audit.py} checks disjoint kernel directions over the native fields, their annihilation of the multilevel algebraic wire observations, the exact fingerprint polynomial identity and the concrete combined error bound. It does not numerically estimate a statistical error of order $2^{-151}$.
