% !TeX root = ../../main.tex
\subsection{A public-randomness route through the bytecode residual}\label{note:zk-bytecode-public-library}
...
\subsubsection{Public random ratios and private choice bits}
...
\subsubsection{Realize the ratios with valid SET cycles}
...
\]
where $J_s$ is the nonzero ratio from the two closing JUMPs and contains no random SET constant. Condition the public SET fingerprints away from $0,\delta$, independently for each word. The affine-coset character argument in \noteref{zk-gkr-paired} bounds each ratio's character mean by $2\sqrt q/(Q^2-2)$. The two independent words consequently give
\[
...
\subsubsection{Fixed public libraries and composition}
