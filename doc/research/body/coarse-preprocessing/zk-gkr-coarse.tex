% !TeX root = ../../main.tex
\subsection{A first-packet GKR leak and an executed-payload repair}\label{note:zk-gkr-coarse}

\paragraph{Result and scope.} The masks of \noteref{zk-bus-root-joint} can leave an entire MUL block product untouched in the first GKR packet. Thirty-two random valid MUL cycles hide that push child jointly with the shared root and seventeen boundary values. The continuation \noteref{zk-gkr-paired} adds its pull counterpart. Other GKR messages remain open; coins are honest interactive challenges, not Fiat-Shamir.

\subsubsection{An actual unmasked child}
...
\subsubsection{Random MUL payloads repair this push child}
