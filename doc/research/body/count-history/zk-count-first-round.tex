% !TeX root = ../../main.tex
\subsection{Jointly hiding the first count round and the final children}\label{note:zk-count-first-round}
...
\subsubsection{The first endpoints are already known}
...
\subsubsection{Six separated banks}

Use the $448$-point set $S\subset\cube{12}$ from \noteref{zk-stacked-padding}: six arbitrary low bits and six further bits of weight at most one. Each bank has two trade bits $u_{b,0,s},u_{b,1,s}$ for every $s\in S$. In an aligned $2^{19}$-row count column, the positions are
\[
 2^{16}b+8s+4t+2^{15}\sigma+i,
 \quad t,\sigma\in\{0,1\},\quad i\in\{0,1,2,3\}.
\]
Here $t$ is the first folded coordinate, $\sigma$ separates the exchanged quartets, and the top three bits select one of six banks. For each fixed $(b,s,t)$, a fair bit exchanges the ordered counter pairs $(1,\gen^3)$ and $(\gen,\gen^2)$ between $\sigma=0,1$, in the selected child positions $(i,j)$. This is the balanced trade of \noteref{zk-count-children}.
...
\subsubsection{A triangular joint simulation}
...
\paragraph{Remaining work.} This particular library does not hide the entire layer: its backgrounds become constant after the first fold, leaving a witness-dependent second-round cubic coefficient unmasked. \noteref{zk-count-middle-rounds} gives the valid-complement counterexample and develops all-coordinate backgrounds with twisted label exchanges to address this and a separator invariant. The intervening rounds consume both bit families, so the seven-coordinate theorem cannot be composed by treating those bits as fresh. Trades also change fingerprint-tree messages; the actual batched bus prefix, commitments and Fiat-Shamir remain outside this model.

\paragraph{Evidence.} \auditref{zk_count_first_round_audit.py} checks the endpoint identities in odd- and even-depth complete GKR replays, the actual-field joint binary rank $1344$, the rational error bound, and the original and randomized $43008$-row libraries against the actual ISA bus forms. The replays use identity fingerprint trees, not a full VM proof. The script imports the reusable wire replay from \auditref{zk_count_children_audit.py}.
