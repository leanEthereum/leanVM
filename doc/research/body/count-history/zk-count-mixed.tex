% !TeX root = ../../main.tex
\subsection{Mixed-bank leakage and fixed counter anchors}\label{note:zk-count-mixed}
...
\subsubsection{Twisting still leaves a common factor}
...
\subsubsection{Two fixed anchors repair the root pair}
...
The script also checks binary rank $384$ for the full-support two-anchor root pair, the rational bound, and valid anchor placements outside the random support. The nonlinear obstruction in \noteref{zk-count-tail} requires a new placement before a complete-layer simulator can be sought. The restricted root repair must not be composed with earlier marginal results by reusing the same randomness.
