...
\paragraph{What this does not compose.} The six products determine the banks' contribution to both node-$13$ products, and the SET state products are the reversed JUMP state products. But SET bytecode products, memory finalization and later messages still depend jointly on the scales and private choices. They cannot be retained by merely counting their output bits. Nor does the old public-word theorem, which conditions on a different view, automatically compose with this theorem. Full first/second-layer simulation with the previous boundary remains the next obligation.
