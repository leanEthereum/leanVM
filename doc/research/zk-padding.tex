...
\paragraph{Bus.} The initial root and packet results are extended in \path{zk-bus-boundary.tex}: valid counter trades, XOR payloads and loop-occupancy choices jointly hide all twelve final GKR children and all five following framework evaluations below $2^{-156}$. This repairs the memory-disclosure residual of \path{zk-bus-packet.tex} and covers the derived table-bus targets. The continuation \path{zk-bus-root-joint.tex} includes the shared root jointly below $2^{-151}$, retaining the actual linear disclosures of the same thirty-two unused words. A publicly normalized count root can be adjoined. The newer \path{zk-gkr-coarse.tex} exhibits an untouched MUL product in the first packet of a supported layout. Its continuation \path{zk-gkr-paired.tex} jointly hides both corresponding push/pull children, the shared root and seventeen boundary values below $2^{-155}$ using forty random MUL cycles and a paired character-sum bound. The remaining first packet and deeper GKR layers still need joint treatment; there is no full bus simulator.
