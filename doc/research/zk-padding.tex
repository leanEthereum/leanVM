...
\paragraph{Bus and Flock.} Eight unused random memory words now provably hide the shared bus product root: \path{zk-bus-root.tex} derives a bound below $2^{-151}$ using the actual fingerprint and multiplicative character sums. Their joint distribution with GKR and memory openings remains unproved. For Flock, \path{zk-flock-skip.tex} proves joint hiding of the first $64$-value message and eight within-block zerocheck rounds below $2^{-157}$. A certified library of valid BLAKE2s compressions spans their $79$-dimensional public message space; fair choices between those compressions, repeated on the sparse support, provide extension-field noise without a pseudorandomness assumption. The construction reserves $57344$ random-choice compressions in a public batch of $2^{19}$. Its joint distribution with the preceding VM messages, batch-coordinate zerocheck rounds, lincheck and terminal slices remains unproved.
