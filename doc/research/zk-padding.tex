...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} includes all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. The nonlinear-side-information extension \path{zk-public-seed-leakage.tex} retains twelve control fields. The new \path{zk-set-payload-products.tex} also treats the correlated SET payload products: a subfield character bound decouples seven free product coordinates below $2^{-159}$, reconstructing dependent products by balance. The actual controls and complementary cofactors still need simulation; the earlier boundary simulator does not automatically extend to them. Public preprocessing changes the program digest and adds an honest-setup assumption; it has not been adopted or implemented. Mixed GKR messages, deterministic padding libraries, later proofs and Fiat-Shamir remain unresolved.
