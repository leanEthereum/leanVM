...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} uses four compact banks to include all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. Its multi-product Fourier bound retains the correlated evaluation. The new \path{zk-public-seed-leakage.tex} removes the linearity restriction: larger banks decouple these products from twelve nonlinear control fields below $2^{-159}$, retaining their actual law rather than simulating it. This does not extend the earlier boundary simulator to those controls. Public preprocessing changes the program digest and adds an honest-setup assumption; it has not been adopted or implemented. Mixed GKR messages, deterministic padding libraries, later proofs and Fiat-Shamir remain unresolved.
