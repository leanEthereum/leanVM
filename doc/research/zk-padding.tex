...
\paragraph{Optional public preprocessing.} The branch \path{zk-bytecode-public-library.tex} closes the bytecode residual using private occupancy bits and public random SET constants installed once in bytecode. The joint product/boundary reduction is established in \path{zk-set-payload-products.tex} and \path{zk-control-residual.tex}, retaining correlated controls. Public frame scalings in \path{zk-frame-gauge.tex} and their ordered composition in \path{zk-control-composition.tex} leave seven actual residual fields. The payload banks of \path{zk-even-children.tex} cover three child pairs but consume their surviving product randomness. The new \path{zk-second-frontier.tex} spreads three further MUL banks across the other memory quarters. An integer unit certificate makes the entire second frontier uniform subject to balance, four public bytecode nodes and six actual nodes. This covers the complete first two GKR layers, including both mixed round polynomials, jointly with the final boundary, below $2^{-155}$ outside a setup exception below $2^{-222}$. The new residual is eight fields: MUL state/bytecode products on both channels, two JUMP condition/destination products and the final-counter evaluations. Its joint law remains unsimulated. The auxiliary products from the earlier strengthened views are no longer retained. This branch requires native field-valued frame/offset support and an honestly sampled public library that changes the program digest. Neither has been adopted or implemented. Full GKR, deterministic libraries, later proofs and Fiat-Shamir remain unresolved.
