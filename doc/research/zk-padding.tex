...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} includes all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. The nonlinear extension \path{zk-public-seed-leakage.tex} retains twelve control fields; \path{zk-set-payload-products.tex} adds the correlated SET products. The joint reduction \path{zk-control-residual.tex} leaves thirteen actual residual fields, including an outside bytecode ratio unchanged by SET padding. The new \path{zk-gkr-second-wire.tex} instead examines the actual mixed layer: exact wire symmetries and derivative certificates leave the MUL bytecode-node ratio undetermined on arbitrary balanced frontiers. These frontiers are not proved realizable by VM padding, so this is not a privacy bound. The continuation \path{zk-frame-gauge.tex} jointly decouples six JUMP products below $2^{-159}$ while retaining eight fields, but excludes other correlated messages and needs native field-valued frame/offset support. Public preprocessing changes the program digest and adds an honest-setup assumption; it has not been adopted or implemented. Full GKR, deterministic libraries, later proofs and Fiat-Shamir remain unresolved.
