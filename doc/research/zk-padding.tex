...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} uses four compact banks to include all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. Its multi-product Fourier bound retains the correlated evaluation. This changes the program digest and adds an honest-setup assumption; it has not been adopted or implemented. The grandchildren are a strengthened view, not individually transmitted values in the next layer. Mixed GKR messages, deterministic padding libraries, later proofs and Fiat-Shamir remain unresolved.
