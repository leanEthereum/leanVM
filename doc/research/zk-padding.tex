...
\paragraph{Bus.} Eight unused random memory words hide the shared bus product root: \path{zk-bus-root.tex} gives error below $2^{-151}$ by a character-sum argument. Its continuation, \path{zk-bus-gkr-boundary.tex}, supplies jointly uniform internal nodes and identifies the payload-free push/pull child differences. The newer \path{zk-bus-packet.tex} hides all twelve final GKR children jointly below $2^{-157}$, using independent frame and bytecode counter trades and thirty-two unused cells. However, subsequent memory evaluations expose a residual that cancels this whole mask family and still depends on executed private payloads. The preceding GKR transcript and this residual require additional joint treatment; there is no full bus theorem.
...
\paragraph{The full table batch and the next step.} \path{zk-table-batch.tex} gives an exact conditional simulator for the actual six-table sumcheck, including unequal heights. One JUMP inverse bank covers its free round coordinates: JUMP is the only table with explicit constraints, while the other tables contribute quadratic bus forms. The triangular counter construction in \path{zk-jump-bus-padding.tex} now hides the JUMP component's bus-batched sumcheck, initial target and all fourteen terminal evaluations with error below $2^{-146}$. It does not condition on preceding GKR messages. The missing full-batch input is still a joint simulator for GKR, table bus targets and terminal/residual data. Memory claims, other tables' terminal data, access counts, Flock and PCS need joint treatment as well. No full ZK or novelty claim is made; production protocol code remains unchanged.
