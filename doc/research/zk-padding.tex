...
\paragraph{Inside the count tree.} GKR's final four child messages expose an evaluation of the quartet-sum array, which shuffling within quartets cannot mask. \path{zk-count-first-round.tex} proves joint hiding of the first count polynomial and final four children below $2^{-157}$, using $43008$ valid rows; its first endpoints are already known from the preceding packet. Intermediate-round leaks require geometric backgrounds and twisted trades, developed in \path{zk-count-middle-rounds.tex}. The full mixed-term audit in \path{zk-count-mixed.tex} then finds a common-root leak, repairs the pair of root evaluations with fixed valid anchors, and checks a finite full-layer affine span. Nevertheless, \path{zk-count-tail.tex} proves a nonlinear endpoint leak in that anchored six-bank layout. The replacement in \path{zk-count-suffix.tex} gives each suffix group four independent directions, jointly hiding its whole child table, the subsequent count rounds and their incoming claim below $2^{-157}$. Additional conditional invariants require the parity pivots of \path{zk-count-head.tex}. The actual sparse-bit matrix in \path{zk-count-full-head.tex} reaches the full conditional rank for all seventeen head rounds at one certified completion and coin assignment. The trace-dual method in \path{zk-count-dual.tex} reduces the post-sparse surjectivity test to two elimination steps, but a parameter-uniform symbolic certificate is still needed. A uniform rank-failure bound, preceding GKR layers and the actual fingerprint channels remain open.

\paragraph{Bus and Flock.} Eight unused random memory words now provably hide the shared bus product root: \path{zk-bus-root.tex} derives a bound below $2^{-151}$ using the actual fingerprint and multiplicative character sums. Their joint distribution with GKR and memory openings remains unproved. Flock is harder: a padding compression must satisfy the fixed Boolean BLAKE2s circuit, and its univariate skip already exposes $64$ field elements. Random inputs are not a proof of statistical hiding. A possible algebraic approach is a library of valid compression witnesses whose differences provide certified Boolean directions, followed by analysis of their nonlinear messages. Treating compression as a pseudorandom expander would not establish the requested statistical guarantee.
