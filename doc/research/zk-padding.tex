...
\paragraph{Beyond the coarse frontier.} \path{zk-frontier-lifting.tex} supplies twelve product-preserving directions, but full third-layer simulation remains open; native leaf relations rule out unrestricted uniform-frontier induction. A different route uses prefix-preserving row swaps. The continuation \path{zk-penultimate-round.tex} combines eighteen sparse banks, three bounded-degree determinants and quartic smoothing. It jointly hides the last two sumcheck rounds of the final GKR layer, their incoming claim and boundary below $2^{-153}$, extending the last-round theorem with a uniform rank bound. The first eighteen rounds within the final layer are excluded; composition with the coarse theorem remains unproved.
