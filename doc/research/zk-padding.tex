...
\paragraph{Beyond the coarse frontier.} \path{zk-frontier-lifting.tex} supplies twelve product-preserving directions, but full third-layer simulation remains open; native leaf relations rule out unrestricted uniform-frontier induction. A different route uses prefix-preserving row swaps. The latest continuation, \path{zk-four-round-tail.tex}, respects five native read relations and a private self-loop invariant. Forty-eight sparse banks and bounded-degree determinants jointly hide the last four sumcheck rounds of the final GKR layer, their incoming claim and boundary below $2^{-152}$. Four independent coupled-quartic sources hide the new round; successive factorizations retain the earlier tail without reusing randomness. The first sixteen rounds within that layer and composition with the coarse theorem remain open.
