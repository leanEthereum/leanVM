...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} includes all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. The nonlinear extension \path{zk-public-seed-leakage.tex} retains twelve control fields; \path{zk-set-payload-products.tex} adds the correlated SET products. The joint reduction \path{zk-control-residual.tex} leaves thirteen actual residual fields. Public frame scaling in \path{zk-frame-gauge.tex} supplies further control randomness. The ordered composition \path{zk-control-composition.tex} reduces that residual to seven fields and handles the actual second child-$1$ pair after removing two auxiliary state products. Adding thirty-six XOR cycles in \path{zk-odd-children.tex} covers both odd-child pairs. A second $48$-word MUL input bank in \path{zk-even-children.tex} includes child $2$ jointly, below $2^{-155}$ outside a setup exception below $2^{-222}$. At fixed cofactors, children $2,3$ recover all surviving payload-product randomness; child $0$ and mixed round polynomials remain excluded. The actual-wire ambiguity of \path{zk-gkr-second-wire.tex} is still not a full privacy bound. This branch requires native field-valued frame/offset support; public preprocessing changes the program digest and assumes an honestly sampled library. Neither has been adopted or implemented. Full GKR, deterministic libraries, later proofs and Fiat-Shamir remain unresolved.
