...
\paragraph{Beyond the coarse frontier.} \path{zk-frontier-lifting.tex} supplies twelve product-preserving directions at the next fingerprint frontier, retaining actual counts and boundary below $2^{-165}$. Full third-layer simulation still needs further fingerprint and joint refined-count arguments. Exact push/pull relations rule out unrestricted uniform-frontier induction to the leaves. The alternative \path{zk-subtree-swaps.tex} gives prefix-preserving binary-affine noise, with an exact instance retaining $448$ bits after the boundary. \path{zk-swap-terminal.tex} locates these bits before the last quartic. Its six-field endpoint target is closed by \path{zk-terminal-boundary.tex}: enlarged XOR and occupancy banks jointly hide the last quartic, incoming claim and boundary below $2^{-156}$. Earlier wire is excluded; composition with the coarse theorem remains unproved.
