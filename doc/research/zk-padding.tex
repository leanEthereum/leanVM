...
\paragraph{Bus and Flock.} Eight unused random memory words now provably hide the shared bus product root: \path{zk-bus-root.tex} derives a bound below $2^{-151}$ using the actual fingerprint and multiplicative character sums. Their joint distribution with GKR and memory openings remains unproved. For Flock, \path{zk-flock-skip.tex} proves marginal hiding of the first $64$-value message below $2^{-157}$. A certified library of valid BLAKE2s compressions spans its $63$-dimensional public message space; fair choices between those compressions, repeated on the sparse support, provide extension-field noise without a pseudorandomness assumption. The construction reserves $28672$ random-choice compressions in a public batch of $2^{18}$. Its joint distribution with the preceding VM messages, later Flock zerocheck and lincheck, and terminal slices remains unproved.
