...
\paragraph{Optional public preprocessing.} The branch \path{zk-bytecode-public-library.tex} closes the bytecode residual using private occupancy bits and public random SET constants installed once in bytecode. The joint product/boundary reduction is established in \path{zk-set-payload-products.tex} and \path{zk-control-residual.tex}, retaining correlated controls. Public frame scalings in \path{zk-frame-gauge.tex} and their ordered composition in \path{zk-control-composition.tex} leave seven actual residual fields. The payload banks of \path{zk-even-children.tex} cover three child pairs but consume their surviving product randomness. Spreading MUL banks across the other memory quarters in \path{zk-second-frontier.tex} gives a full balanced-frontier reduction with eight residual fields. The new \path{zk-mul-control.tex} closes that residual: an independent public MUL scale library masks four control products, private nonzero JUMP conditions mask two more, and the existing occupancy masks then hide both final counters. The complete first two GKR layers, including both mixed round polynomials, and the final seventeen-field boundary now have a joint public-data simulator below $2^{-155}$ outside a setup exception below $2^{-222}$. There is no supplied witness-dependent residual for this projected view. Its auxiliary product disclosures and the omitted intermediate GKR transcript are not simulated. This branch requires native field-valued frame/offset support and honestly sampled public libraries that change the program digest. Neither has been adopted or implemented. Full GKR, deterministic libraries, later proofs and Fiat-Shamir remain unresolved.
