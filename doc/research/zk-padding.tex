...
\paragraph{Optional public preprocessing.} The separate branch \path{zk-bytecode-public-library.tex} closes that residual using $1024$ private occupancy bits and independent public random SET constants installed once in bytecode. The continuation \path{zk-bytecode-frontier.tex} includes all four bytecode grandchildren jointly with the first-packet/boundary view, below $2^{-154}$ outside a setup exception below $2^{-160}$. The nonlinear extension \path{zk-public-seed-leakage.tex} retains twelve control fields; \path{zk-set-payload-products.tex} adds the correlated SET products. The new \path{zk-control-residual.tex} composes these with the first packet and boundary, reducing the enlarged view to thirteen actual residual fields below $2^{-155}$. Its extra residual is a bytecode ratio from the other opcodes, which SET padding leaves unchanged and which a valid local pair separates. This is a limitation of the strengthened view, not an attack on the mixed GKR messages. Public preprocessing changes the program digest and adds an honest-setup assumption; it has not been adopted or implemented. Full GKR, deterministic libraries, later proofs and Fiat-Shamir remain unresolved.
