...
\paragraph{Result and scope.} The masks of \path{zk-bus-root-joint.tex} can leave an entire MUL block product untouched in the first GKR packet. Thirty-two random valid MUL cycles hide that push child jointly with the shared root and seventeen boundary values. The continuation \path{zk-gkr-paired.tex} adds its pull counterpart. Other GKR messages remain open; coins are honest interactive challenges, not Fiat-Shamir.
