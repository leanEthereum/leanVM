...
\begin{itemize}
  \item \textbf{stateless} with a lifetime of $2^{24}$ signatures.
  ...
  \item \textbf{1.38M hashes at key generation}.
  \item \textbf{190K hashes at signing}, with 1024 bytes of cached state at key generation (not secret)
\end{itemize}
\end{abstract}
...
The ideal SPHINCS scheme specified above, with independently sampled secrets, has 127 bits of classical strong unforgeability in the ROM at $\qs=2^{24}$ signatures.
\end{theorem}
