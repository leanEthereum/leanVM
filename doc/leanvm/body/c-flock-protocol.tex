...
leanVM runs a second circuit the same way, the Keccak step of the \texttt{SHA3} instruction (\S\ref{sec:tab-sha3}), with its own witness, its own $\qflock$ and its own run of this protocol, right after the BLAKE2s one on the same transcript. Everything in this annex applies to it with its own numbers: $\kblock=16$, the constant wire at position $3200$, $10$ lincheck rounds. Its committed wires are $1600$ free inputs, the constant, $1600$ committed $\oplus$ wires carrying the output state, and $38{,}400$ $\wedge$ wires, one per bit of $\chi$ in each of the $24$ rounds (the multiplicative complexity of $\chi$ is exactly one product per bit), so its matrix walk costs $80{,}002$ field multiplications. Its soundness error, by the argument closing this annex, is $(4\kbatch+173)/|\E|$, and the two circuits' errors add.
