...
\subsection{Evaluation by blocks}\label{sec:ntt-blocks}

LeanDA checks the inner products $\inner{\dual}{w_i}$ by processing the encoded rows one cell column at a time (\S\ref{sec:da-circuit}). It therefore needs the weights $\dual(x)$ for one cell column, shared across all rows. The following lemma computes just those weights with a small transform. The higher-degree factors of $\dual$ are constant within the cell, so they require no transform layers.

\begin{lemma}[Cost of evaluating a block]\label{lem:ntt-blocks}
Let $\dual$ be the polynomial of \eqref{eq:dual-product}, on a domain of size $\blen=2\kdim$. Let $\cellsz\le\kdim$ be a power of two. After precomputing tables of $O(\cellsz+\log_2^2\blen)$ field elements, depending only on the basis and dimensions, all $\cellsz$ values of $\dual$ on any block $b+\subsp_{\log_2\cellsz}$ can be computed in
\[
  O\bigl(\cellsz\log_2\cellsz+\log_2^2\blen\bigr)
\]
field operations. The transform part is one size-$\cellsz$ additive NTT, with exactly $\tfrac12\cellsz\log_2\cellsz$ butterflies.
\end{lemma}

\begin{proof}
Choose the block offset $b$ with zero coordinates below $\log_2\cellsz$, and write each point as $x=b+u$ with $u\in\subsp_{\log_2\cellsz}$. Linearity gives $\Wn{j}(b+u)=\Wn{j}(b)+\Wn{j}(u)$. For $j\ge\log_2\cellsz$, the polynomial $\Wn{j}$ vanishes on this subspace, so
\[
\begin{split}
  \dual(b+u)
  ={}&\prod_{\log_2\cellsz\le j<\log_2\kdim}
       \bigl(1+z_j\Wn{j}(b)\bigr)\\
     &\quad\cdot\prod_{j<\log_2\cellsz}
       \bigl(1+z_j\Wn{j}(b)+z_j\Wn{j}(u)\bigr).
\end{split}
\]
The first product is a scalar shared by the whole block. The second has coefficient tensor $\bigotimes_{j<\log_2\cellsz}(1+z_j\Wn{j}(b),z_j)$ in the novel basis on the fixed subspace $\subsp_{\log_2\cellsz}$.

Precompute the transform's $\cellsz-1$ twiddles and the values $\Wn{j}(e_c)$ for $j<\log_2\kdim$ and $\log_2\cellsz\le c<\log_2\blen$. For each block, summing the entries selected by the bits of $b$ computes all $\Wn{j}(b)$ in $O(\log_2\kdim\,\log_2(\blen/\cellsz))\subseteq O(\log_2^2\blen)$ additions. Forming the scalar prefactor costs $O(\log_2\kdim)$ operations. Expanding the tensor, with that scalar as its initial coefficient, costs $O(\cellsz)$ operations. Finally, Proposition~\ref{prop:ntt} evaluates it on the block in $\tfrac12\cellsz\log_2\cellsz$ butterflies. The same twiddles serve every block.
\end{proof}

There are $\blen/\cellsz$ blocks. Computing each block's weights once and using them for all $\nblob$ rows therefore evaluates every membership inner product in
\[
  O\!\left(
    \blen\log_2\cellsz
    +\frac{\blen}{\cellsz}\log_2^2\blen
    +\nblob\blen
  \right)
\]
field operations, after precomputation. The last term is one multiplication and one addition per matrix entry. For LeanDA, each block contains $256$ symbols, so the shared weight computation uses $256$-point transforms instead of one transform over all $32768$ domain points. This avoids the upper transform layers and their intermediate writes in the VM's write-once memory.
