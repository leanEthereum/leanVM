...
\texttt{SHA3}           & $[o_{m_0},\dots,o_{m_7},o_{\mathit{cap}},o_{\mathit{out}},\fdig]$ & one Keccak sponge step (see below)\\
\hline
\end{tabularx}
\end{center}
...
\texttt{SHA3} is one step of the Keccak sponge~\cite{fips202}, the second hash instruction, there for Keccak-256 (\S\ref{sec:tab-sha3}). It reads a $1600$-bit state of $25$ lanes of $64$ bits from thirteen memory cells, XORs the padding's last bit $\mathtt{0x80}\ll 56$ into lane $16$, applies the Keccak-$f[1600]$ permutation, and writes the $25$ resulting lanes to thirteen cells in the same layout. A cell holds two lanes through the canonical embedding $a_0+a_1y\mapsto a_0+a_1y+0y^2$, so using it as a \texttt{SHA3} operand constrains its top limb to zero, except for the one cell holding lane $16$ alone, whose high limb is zero too. The eight cells holding lanes $0..16$, a whole block, are addressed \emph{independently} by references $o_{m_0},\dots,o_{m_7}$ (cell $i$ at address $\fp\cdot o_{m_i}$), and the five holding lane $16$ and the capacity lanes $17..25$ are the consecutive cells $\loc{\gen^k\cdot o_{\mathit{cap}}}$, $k<5$. The result goes to the consecutive cells $\loc{\gen^k\cdot o_{\mathit{out}}}$, $k<13$, so each row accesses twenty-six cells, or with the flag $\fdig=1$ (a \emph{digest step}) to the first two alone, $k<2$. The digest of a hash is the first two output cells, so a final block needs nothing else.
