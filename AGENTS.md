...
- **NEON is the width ceiling on Apple silicon**, so an AVX-512 win that is purely width has no counterpart: the M4 has no SVE, and its SME2 is streaming-mode matrix work with no polynomial multiply. What does port is *shape*. A fused NTT pass on NEON wants a butterfly at a time over whole rows: a tile leaves only its own width of independent work to cover the reduction's dependent PMULL folds, where a row leaves the whole lane count. AVX-512 uses register-resident tiles. Both preserve coefficient order because base-field twiddles scale every coefficient independently. Measured both directions on NEON: the tile costs the extension NTT, and costs the base encode's `Commit` again.
