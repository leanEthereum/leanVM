...
\begin{enumerate}
    ...
    \item \textbf{Logup.}
    \begin{enumerate}
        \item  The verifier samples $\gamma \in \Fq$ and $\vec\alpha \in \Fq^{\ell}$ (bus-tuple hashing seeds, $\ell = \log_2 \textsf{bus\_width}$), used for Equation \ref{eq:logup-sum}.
    ...
    \item \textbf{AIR.}
    \begin{enumerate}
        \item The verifier samples $\alpha_\textsc{air} \in \Fq$. Consecutive powers $1, \alpha_\textsc{air}, \alpha_\textsc{air}^2, \dots$ are used to take a random linear combination of \emph{all} verification constraints across the three tables.
