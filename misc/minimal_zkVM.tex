...
\paragraph{Addressing-mode constraints.}
\begin{itemize}
...
\paragraph{Opcode selectors.} Let $P_0, P_1, P_2$ be the Lagrange basis polynomials at $0, 1, 2$:
$$P_0(x) = (x-1)(x-2)/2, \qquad P_1(x) = x(2-x), \qquad P_2(x) = x(x-1)/2.$$
We define the selectors
\begin{itemize}
    \item $\textsf{add} = P_1(\textsf{aux}_1)$ \quad (equals $1$ when $\textsf{aux}_1 = 1$, else $0$ on $\{0, 2\}$),
    \item $\textsf{deref} = P_2(\textsf{aux}_1)$ \quad (equals $1$ when $\textsf{aux}_1 = 2$, else $0$ on $\{0, 1\}$),
    \item $\textsf{is\_precompile} = 1 - (\textsf{add} + \textsf{flag}_\textsf{mul} + \textsf{deref} + \textsf{flag}_\textsf{jump})$.
\end{itemize}
$\textsf{is\_precompile}$ is used as the bus selector for precompile dispatch (see Section~\ref{buses}).

\paragraph{Per-opcode semantic constraints.}
\begin{itemize}
    \item $\texttt{ADD}$: $\quad \textsf{add} \cdot (\nu_B - (\nu_A + \nu_C)) = 0$.
    \item $\texttt{MUL}$: $\quad \textsf{flag}_\textsf{mul} \cdot (\nu_B - \nu_A \cdot \nu_C) = 0$.
    \item $\texttt{DEREF}$: $\quad \textsf{deref} \cdot (\textsf{addr}_B - (\textsf{value}_A + \textsf{operand}_B)) = 0$ \quad and \quad $\textsf{deref} \cdot (\textsf{value}_B - \nu_C) = 0$.
...
At each cycle when $\textsf{is\_precompile} = 1$, the execution table \textsc{Push}es $$\sigma = (\textsf{aux}_2, \nu_A, \nu_B, \nu_C)$$ to the "precompile bus".
...
Similarly to the $\textsf{is\_precompile}$ (virtual) column for the execution table, each one of the two precompile tables has a boolean (virtual) column acting as a bus selector, indicating at which rows the table is pulling from the bus (1) or not (0).
