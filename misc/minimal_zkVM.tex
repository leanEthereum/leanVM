...
Cconstraints have degree 5.

\vspace{5mm}

We define the following values:
\begin{itemize}
    \item $\nu_A = \textsf{flag}_A \cdot \textsf{operand}_A + (1 - \textsf{flag}_A - \textsf{flag}^{AB}_\textbf{fp}) \cdot \textsf{value}_A + \textsf{flag}^{AB}_\textbf{fp} \cdot (\textbf{fp} + \alpha)$ 
    \item $\nu_B = \textsf{flag}_B \cdot \textsf{operand}_B + (1 - \textsf{flag}_B - \textsf{flag}^{AB}_\textbf{fp}) \cdot \textsf{value}_B + \textsf{flag}^{AB}_\textbf{fp} \cdot (\textbf{fp} + \beta)$
    \item $\nu_C = \textsf{flag}_C \cdot \textsf{operand}_C + (1 - \textsf{flag}_C - \textsf{flag}_\textbf{fp}) \cdot \textsf{value}_C + \textsf{flag}_\textbf{fp} \cdot (\textbf{fp} + \gamma)$
\end{itemize}
