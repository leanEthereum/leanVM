...
\[
\texttt{EXTENSION\_OP}[\;\alpha^\textsf{addr},\, \alpha,\;\; \beta^\textsf{addr},\, \beta,\;\; \gamma^\textsf{addr},\, \gamma,\;\; \texttt{op},\;\; \textsf{be},\;\; N\;]
\]
...
\begin{itemize}
    \item $\alpha, \beta, \gamma \in \Fp$
    \item $\alpha^\textsf{addr}, \beta^\textsf{addr}, \gamma^\textsf{addr} \in \{\texttt{imm}, \texttt{mem}, \texttt{fp\_rel}\}$, with $\alpha^\textsf{addr} = \texttt{fp\_rel} \Leftrightarrow \beta^\textsf{addr} = \texttt{fp\_rel}$
    \item $\texttt{op} \in \{\textsc{add}, \textsc{dot\_product}, \textsc{eq}\}$, $\textsf{be} \in \{0, 1\}$
    \item $N \geq 1$
\end{itemize}
...
a_i =
\begin{cases}
\textbf{m}[a + i] \in \Fp & \text{if } \textsf{be} = 1 \quad \text{(base-extension)} \\[1mm]
\mathsf{embed}\big(\textbf{m}[a + 5i \,..\, a + 5(i + 1)]\big) \in \Fq & \text{if } \textsf{be} = 0 \quad \text{(extension-extension)}
\end{cases}
\]
...
\begin{itemize}\setlength\itemsep{4pt}
    ...
    \item $\texttt{POSEIDON\_OP}[\alpha^\textsf{addr}, \alpha, \beta^\textsf{addr}, \beta, \gamma^\textsf{addr}, \gamma, \texttt{permute}, \texttt{half}, \delta]$: $\textsf{aux}_2 \,=\, 1 + 2\,\texttt{permute} + 4\,\texttt{half} + 8 \cdot [\delta \neq \bot] + 16\, \delta \cdot [\delta \neq \bot]$;\; $\nu_A \!\leftarrow\! (\alpha^\textsf{addr}, \alpha)$, $\nu_B \!\leftarrow\! (\beta^\textsf{addr}, \beta)$, $\nu_C \!\leftarrow\! (\gamma^\textsf{addr}, \gamma)$.
    \item $\texttt{EXTENSION\_OP}[\alpha^\textsf{addr}, \alpha, \beta^\textsf{addr}, \beta, \gamma^\textsf{addr}, \gamma, \texttt{op}, \textsf{be}, N]$: $\textsf{aux}_2 \,=\, 2\,\textsf{be} + 4\,[\texttt{op} = \textsc{add}] + 8\,[\texttt{op} = \textsc{dot\_product}] + 16\,[\texttt{op} = \textsc{eq}] + 32 N$;\; $\nu_A \!\leftarrow\! (\alpha^\textsf{addr}, \alpha)$, $\nu_B \!\leftarrow\! (\beta^\textsf{addr}, \beta)$, $\nu_C \!\leftarrow\! (\gamma^\textsf{addr}, \gamma)$.
\end{itemize}
...
Poseidon \cite{poseidon1} is a permutation of $\Fp^{16}$. The table runs this permutation once per row, and the final part of the row, which is exposed to memory via lookup, is a function of a compile-time bit $\textsf{flag\_permute}$: we perform either a $16 \to 8$ \emph{compression} or a $16 \to 16$ \emph{permutation}.
...
\paragraph{Compression mode} ($\textsf{flag\_permute} = 0$) Using pairwise addition in $\Fp$:
...
\paragraph{Permute mode} ($\textsf{flag\_permute} = 1$). 
...
We introduce a boolean flag $\textsf{flag\_short}$ that signals that only the first $\texttt{n}$ field elements of the output should be written to memory, at $\textbf{m}[\nu_C..\nu_C + \texttt{n}]$ (the remaining $8 - \texttt{n}$ ones are ignored), 

\vspace{5mm}

\textbf{2. Custom first-half of left-input}

We introduce a boolean flag $\textsf{flag\_left}$, alongside a compile-time parameter $\textsf{offset\_left}$:
$$\texttt{left} = \begin{cases} \textbf{m}[\nu_A..\nu_A + 8] & \text{if } \textsf{flag\_left} = 0 \text{ (default)} \\ \textbf{m}[\textsf{offset\_left}..\textsf{offset\_left} + 4] \;\|\; \textbf{m}[\nu_A..\nu_A + 4] & \text{if } \textsf{flag\_left} = 1 \end{cases}$$
...
Both flags ($\textsf{flag\_short}$ and $\textsf{flag\_left}$) and the offset ($\textsf{offset\_left}$) can be encoded in a single (compile-time) parameter, as follows:
$$\textsf{aux}_2 = 1 + 2 \cdot \textsf{flag\_permute} + 4 \cdot \textsf{flag\_short} + 8 \cdot \textsf{flag\_left} + 16 \cdot \textsf{offset\_left} \cdot  \textsf{flag\_left}$$
...
\begin{itemize}
    ...
    \item if $\textsf{flag\_left} = 0$: then $\textsf{offset\_left}$ is unconstrained: but it does not contribute to the encoding $\textsf{aux}_2$, and it has no effect in the AIR logic.
    \item if $\textsf{flag\_left} = 1$: then for the lookup into memory $\textbf{m}[\textsf{offset\_left}..\textsf{offset\_left} + 4]$ to be valid, $\textsf{offset\_left}$ must be smaller than the memory size $M \leq 2^{26} < p / 2$. As a result no overflow can occur in the encoding of $\textsf{aux}_2$, which is thus injective.
\end{itemize}

\textbf{Conclusion.} Every tuple $(\nu_A, \nu_B, \nu_C, \textsf{aux}_2)$ pushed by the execution table is faithfully pulled and decoded by the Poseidon table: the recovered triple $(\textsf{flag\_short}, \textsf{flag\_left}, \textsf{offset\_left})$ matches the one used at the call site, with one harmless exception: when $\textsf{flag\_left} = 0$, $\textsf{offset\_left}$ is left unconstrained, but this is inconsequential since it then has no effect at the AIR level.
...
\textsf{be} & \textsf{start} & $\textsf{f}_\textsf{add}$ & $\textsf{f}_\textsf{mul}$ & $\textsf{f}_\textsf{eq}$ & \textsf{len} & $\textsf{idx}_A$ & $\textsf{idx}_B$ & $\textsf{idx}_R$ & $v_A$ \tiny{(5 cols)} & $v_B$ \tiny{(5 cols)} & \textsf{res} \tiny{(5 cols)} & \textsf{comp} \tiny{(5 cols)} & & $\textsf{aux}_2$ \\
\midrule
...
Notes: $e_i$ denotes the per-element result (virtual, not a committed column). $v_A$ stores 5 base-field coordinates $\textbf{m}[\textsf{idx}_A..\textsf{idx}_A\!+\!5]$; in BE mode only the first coordinate is used, in EE mode all 5 are used. $\textsf{aux}_2$ is a non-committed bus fingerprinting column, equal to $2 \cdot \textsf{be} + 4 \cdot \textsf{flag\_add} + 8 \cdot \textsf{flag\_mul} + 16 \cdot \textsf{flag\_eq} + 32 \cdot \textsf{len}$ on each row. The bus selector is the virtual column $\textsf{activation\_flag} = \textsf{start} \cdot \textsf{active}$ (not shown; equals \textsf{start} on active rows, 0 on padding).
...
\begin{itemize}
    \item \textsf{be} $\in \{0,1\}$: mode flag (1: base $\times$ extension, 0: extension $\times$ extension)
    \item \textsf{start} $\in \{0,1\}$: group boundary (1 on first row of each group and on padding, 0 elsewhere)
    \item $\textsf{flag\_add}$, $\textsf{flag\_mul}$, $\textsf{flag\_eq}$ $\in \{0,1\}$: operation selectors (exactly one is 1 on active rows, all 0 on padding)
    \item \textsf{len}: countdown from $N$ to 1 within each group (1 on padding)
    \item $\textsf{idx}_A$, $\textsf{idx}_B$, $\textsf{idx}_R$: memory addresses
    \item $v_A$ (5 cols). In BE mode only the first coordinate is used; in EE mode all 5.
    \item $v_B$ (5 cols)
    \item \textsf{res} (5 cols) (same on every row of a group)
    \item \textsf{comp} (5 cols): running accumulation; equals \textsf{res} on \textsf{start} rows
\end{itemize}

\textbf{Non-committed columns} (used for bus interaction):
\begin{itemize}
    \item $\textsf{activation\_flag} = \textsf{start} \cdot \textsf{active}$: bus selector (1 on first row of each active group, 0 elsewhere)
    \item $\textsf{aux}_2 = 2 \cdot \textsf{be} + 4 \cdot \textsf{flag\_add} + 8 \cdot \textsf{flag\_mul} + 16 \cdot \textsf{flag\_eq} + 32 \cdot \textsf{len}$
\end{itemize}

\subsubsection{Memory lookups}
\begin{itemize}
    \item $v_A = \textbf{m}[\textsf{idx}_A..+5]$
    \item $v_B = \textbf{m}[\textsf{idx}_B..+5]$
    \item $\textsf{res} = \textbf{m}[\textsf{idx}_R..+5]$
\end{itemize}

\subsubsection{Bus interaction}
On rows where $\textsf{activation\_flag} = 1$ (i.e.\ $\textsf{start} = 1$ and $\textsf{active} = 1$), the table \textsc{Pulls} the tuple $(\textsf{aux}_2, \textsf{idx}_A, \textsf{idx}_B, \textsf{idx}_R)$ from the precompile bus. The execution table \textsc{Pushes} a matching tuple for each EXTENSION\_OP instruction.


The $\textsf{aux}_2$ encoding ensures both the mode and the length are bound to the bus data. Since $\textsf{be}$, $\textsf{flag\_add}$, $\textsf{flag\_mul}$, and $\textsf{flag\_eq}$ are constrained to be boolean, and since the length is constrained to be $\leq 2^{20}$ (by Lemma~\ref{lem:len-bound}), no overflow can occur modulo $p$ and the $\textsf{aux}_2$ value is unique for each combination of parameters, which enforces that all values of $\textsf{aux}_2$ sent by the execution table are correctly received by the extension\_op table.
...
\textbf{Value selection:} In BE mode, $v_A$ is treated as a base field element (only the first of its 5 coordinates matters); in EE mode all 5 coordinates are used. Formally, for $k = 1, \dots, 4$: $v_A[k]$ is multiplied by $(1 - \textsf{be})$ in the constraints, so only $v_A[0]$ survives in BE mode.

Let $\textsf{active} = \textsf{flag\_add} + \textsf{flag\_mul} + \textsf{flag\_eq}$ (1 on active rows, 0 on padding), and define the per-element results (virtual):
\begin{align*}
    e_\text{add} &= v_A + v_B \\
...
Let $\textsf{comp\_tail} = \text{next}(\textsf{comp}) \cdot (1 - \text{next}(\textsf{start}))$ (zero when the next row is a new group).

\vspace{2mm}

\textbf{Boolean constraints:}
\begin{enumerate}
    \item $\textsf{be} \cdot (1 - \textsf{be}) = 0$
    \item $\textsf{start} \cdot (1 - \textsf{start}) = 0$
    \item $\textsf{flag\_add} \cdot (1 - \textsf{flag\_add}) = 0$
    \item $\textsf{flag\_mul} \cdot (1 - \textsf{flag\_mul}) = 0$
    \item $\textsf{flag\_eq} \cdot (1 - \textsf{flag\_eq}) = 0$
\end{enumerate}

\textbf{Accumulation constraint:}
\begin{enumerate}
    \setcounter{enumi}{5}
    \item $\textsf{f}_\textsf{add} \cdot (\textsf{comp} - e_\text{add} - \textsf{comp\_tail}) = 0$ \hfill (additive ADD)
    \item $\textsf{f}_\textsf{mul} \cdot (\textsf{comp} - e_\text{mul} - \textsf{comp\_tail}) = 0$ \hfill (additive MUL)
    \item $\textsf{f}_\textsf{peq} \cdot (\textsf{comp} - e_\text{peq} \cdot (\textsf{comp\_tail} + \text{next}(\textsf{start}))) = 0$ \hfill (multiplicative EQ)
\end{enumerate}
...
\begin{enumerate}
    \setcounter{enumi}{8}
    \item $\textsf{start} \cdot (\textsf{comp} - \textsf{res}) = 0$ \hfill (on start rows, computation equals result)
\end{enumerate}

\textbf{Intra-group consistency:}
\begin{enumerate}
    \setcounter{enumi}{9}
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{len} - \text{next}(\textsf{len}) - 1) = 0$ \hfill (len countdown)
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{be} - \text{next}(\textsf{be})) = 0$ \hfill (mode consistent)
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{f}_\textsf{add} - \text{next}(\textsf{f}_\textsf{add})) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{f}_\textsf{mul} - \text{next}(\textsf{f}_\textsf{mul})) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{f}_\textsf{peq} - \text{next}(\textsf{f}_\textsf{peq})) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\text{next}(\textsf{idx}_A) - \textsf{idx}_A - (\textsf{be} + (1 - \textsf{be}) \cdot 5)) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\text{next}(\textsf{idx}_B) - \textsf{idx}_B - 5) = 0$
\end{enumerate}

\textbf{Boundary:}
\begin{enumerate}
    \setcounter{enumi}{16}
    \item $\text{next}(\textsf{start}) \cdot (\textsf{len} - 1) = 0$ \hfill (when next row is a group start, current len must be 1)
\end{enumerate}
...
\textbf{Base case ($r = H - 1$):} On the wrapping pair (row $H\!-\!1$ paired with itself, see Section~\ref{sec:air-eval}), $\text{next}(x) = x$ for every column $x$. Constraint~10 becomes $(1 - \textsf{start}) \cdot (\textsf{len} - \textsf{len} - 1) = -(1 - \textsf{start}) = 0$, forcing $\textsf{start}_{H-1} = 1$. Then constraint~17 becomes $1 \cdot (\textsf{len} - 1) = 0$, giving $\textsf{len}_{H-1} = 1 = H - (H-1)$.

\vspace{2mm}

\textbf{Inductive step ($r < H - 1$):} Assume $\textsf{len}_s \leq H - s$ for all $s > r$. The pair (row $r$, row $r\!+\!1$) gives two cases depending on $\textsf{start}_{r+1} \in \{0,1\}$ (constraint~2):

\begin{itemize}
    \item If $\textsf{start}_{r+1} = 1$: constraint~17 gives $1 \cdot (\textsf{len}_r - 1) = 0$, so $\textsf{len}_r = 1 \leq H - r$.
    \item If $\textsf{start}_{r+1} = 0$: constraint~10 gives $(1 - 0) \cdot (\textsf{len}_r - \textsf{len}_{r+1} - 1) = 0$, so $\textsf{len}_r = \textsf{len}_{r+1} + 1$ in $\Fp$. By induction, $\textsf{len}_{r+1} \leq H - (r+1)$, so $\textsf{len}_{r+1} + 1 \leq H - r$. Since $H - r \leq H \leq 2^{21} < p$, the addition does not overflow modulo $p$, so $\textsf{len}_r = \textsf{len}_{r+1} + 1 \leq H - r$ as integers.
\end{itemize}
\end{proof}
...
At each cycle when $\textsf{flag}_\textsf{precompile} = 1$, the execution table \textsc{Push}es $$\sigma = (\textsf{aux}_2, \nu_A, \nu_B, \nu_C)$$ to the "precompile bus".

In case of Poseidon call, $\textsf{aux}_2 = 1$ and $\nu_A$, $\nu_B$, $\nu_C$ represent the left input address, the right input address, and the output address (3 pointers to a chunk of 8 field elements in memory).

In case of extension op call, $\textsf{aux}_2 = 2 \cdot \textsf{be} + 4 \cdot \textsf{flag\_add} + 8 \cdot \textsf{flag\_mul} + 16 \cdot \textsf{flag\_eq} + 32 \cdot N$ encodes the mode, the operation, and the length $N$, and $\nu_A$, $\nu_B$, $\nu_C$ represent the starting address of the first operand, the starting address of the second operand, and the address of the result.

Each such tuple $\sigma$ (counting multiplicity) must be \textsc{Pulled} by one of the precompile tables. Given that the Poseidon table only pulls tuples with $\textsf{aux}_2 = 1$, and the extension op table only pulls tuples with $\textsf{aux}_2 \geq 32$, the bus is effectively partitioned into two disjoint sub-buses, one for each precompile.

Similarly to the $\textsf{flag}_\textsf{precompile}$ (virtual) column for the execution table, each one of the two precompile tables has a boolean (virtual) column acting as a bus selector, indicating at which rows the table is pulling from the bus (1) or not (0).
