...
\begin{itemize}\setlength\itemsep{4pt}
    ...
    \]
    \item $\texttt{EXTENSION\_OP}[\alpha^\textsf{addr}, \alpha, \beta^\textsf{addr}, \beta, \gamma^\textsf{addr}, \gamma, \texttt{op}, \textsf{be}, N]$: \; $\nu_A \!\leftarrow\! (\alpha^\textsf{addr}, \alpha)$, $\nu_B \!\leftarrow\! (\beta^\textsf{addr}, \beta)$, $\nu_C \!\leftarrow\! (\gamma^\textsf{addr}, \gamma)$, and
    \[
    \textsf{aux}_2 = 2\,\textsf{be} + 4\,\textsf{flag\_add} + 8\,\textsf{flag\_mul} + 16\,\textsf{flag\_eq} + 32N,
    \]
    where $(\textsf{flag\_add}, \textsf{flag\_mul}, \textsf{flag\_eq})$ is the one-hot encoding of $\texttt{op}$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Intermediate rounds ($68$):} the full state is committed only after each \emph{pair} of full rounds: $\textsf{full}_1, \textsf{full}_2$ ($16$ cols each) after the two pairs making up the $4$ initial full rounds, and $\textsf{full}_3$ ($16$ cols) after the first pair of the $4$ final full rounds (the last pair feeds the output columns directly). Plus $\textsf{sbox}_1, \ldots, \textsf{sbox}_{20}$, the single S-box output (the cube of the first state coordinate) committed at each partial round.
    \item \textbf{Output ($16$):} two halves $\textsf{out}_\text{lo}, \textsf{out}_\text{hi}$ ($8$ cols each). $\textsf{out}_\text{lo}$ holds the written first half — the feed-forward sum $s[0..8] + \texttt{left}$ in compression, or $s[0..8]$ in permutation; $\textsf{out}_\text{hi}$ holds $s[8..16]$ and is used only in permutation.
\end{itemize}
...
\textbf{Precompile pulls.} The table pulls the call tuple
\[
...
On rows where $\textsf{activation\_flag} = \textsf{start} \cdot \textsf{active} = 1$ (the first row of each active group):
\[
\textsc{Pull}\big(\textsf{idx}_A,\, \textsf{idx}_B,\, \textsf{idx}_R,\, 0_{12},\, \textsf{aux}_2\big) \quad \text{with multiplicity } 1,
\]
matching the precompile push issued by the execution table (Section~\ref{sec:execution-bus}) for each \texttt{EXTENSION\_OP} instruction.
