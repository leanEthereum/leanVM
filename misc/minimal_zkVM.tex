...
$h_\textsc{public}$ & variable, $\leq h_\textsc{memory}$ & log2 of the public memory size \\
...
Each VM cycle corresponds to a row of the $\textsc{EXECUTION}$, each poseidon precompile call corresponds to a row of the $\textsc{POSEIDON}$, and each extension precompile call with size parameter $N$ (Section~\ref{extension_op_instruction}) corresponds to $N$ rows of the $\textsc{EXTENSION}$ table.
...
\[
\pi_{\vec\beta}(\sigma) \;=\; \sum_{i=0}^{\textsf{bus\_width} - 1} \sigma_i \cdot \hat{eq}\big(\vec\beta,\, [i]_{01}\big) \;=\; \hat{\sigma}(\vec\beta).
\]
...
In practice, the fingerprinted data $\pi_{\vec\beta}(\sigma_k)$ and multiplicities $m_k$ come from multiple columns, of potentially different sizes. We stack them in a similar way than Section \ref{sec:stacking}. TODO give more details
...
\begin{itemize}
    \item $\textsf{bytecode}$: the bytecode of the program being executed (Section~\ref{sec:opcode-encoding}), of size $2^{h_\textsc{bytecode}}$.
...
\begin{enumerate}
    ...
    The verifier checks: $\textsf{log\_inv\_rate}^{\min} \leq \textsf{log\_inv\_rate} \leq \textsf{log\_inv\_rate}^{\max}$ (i.e.\ $1 \leq \textsf{log\_inv\_rate} \leq 4$); $h^{\min} \leq h_T \leq h_T^{\max}$ for each $T \in \{\textsc{exec}, \textsc{poseidon}, \textsc{extension}\}$ (Section~\ref{sec:table-sizes}); $h_\textsc{memory}^{\min} \leq h_\textsc{memory} \leq h_\textsc{memory}^{\max}$; $h_\textsc{bytecode} \geq h^{\min}$; and $h_\textsc{memory} \geq \max(h_\textsc{bytecode} ,\, h_\textsc{public}\, h_\textsc{exec},\, h_\textsc{poseidon},\, h_\textsc{extension})$.

    \item \textbf{Stacked commitment.} The prover commits via WHIR \cite{whir} to a single multilinear polynomial stacking the memory $\textbf{m}$, the access counts $\textsf{memory\_acc}$ and $\textsf{bytecode\_acc}$, and all committed columns of the three tables.

    \item \textbf{Logup.}
    \begin{enumerate}
        \item  The verifier samples $\gamma \in \Fq$ and $\vec\beta \in \Fq^{\ell}$ (bus-tuple hashing seeds, $\ell = \log_2 \textsf{bus\_width}$), used for Equation~\ref{eq:logup-sum}.
        \item  Run the GKR protocol of Section~\ref{sec:Bus: interractions between table via logup-GKR} to compute the sum of quotients from Equation \ref{eq:logup-sum}. The sum contains terms from: the memory section ($2^{h_\textsc{memory}}$ entries) and the bytecode section ($\max(2^{h_\textsc{bytecode}}, 2^{h_\textsc{exec}})$ entries, padded if $h_\textsc{bytecode} < h_\textsc{exec}$; see Section~\ref{sec:special-pushes}), then, for each table $T$ sorted by descending height, one block of $2^{h_T}$ entries per bus interaction of $T$. Write $h_\textsc{gkr}$ for the log2-ceiling of the resulting total length.
    ...
    \begin{enumerate}
        ...
        \item Prover and verifier run the back-loaded batched zerocheck of Section~\ref{sec:air-eval}, merging the per-table constraint polynomials into a single sumcheck of $h_\textsc{max} := \max_T h_T$ rounds, \emph{crucially reusing $\vec r_\textsc{gkr}$ from the logup step as the zerocheck random challenge (no fresh point is sampled)}. TODO explain why do this, and TODO explain why it's sound. Let $\vec r_\textsc{air} \in \Fq^{h_\textsc{max}}$ denote the sumcheck challenges.
    ...
    \item \textbf{Public-memory consistency.} The verifier samples $\vec r_\textsc{pub} \in \Fq^{h_\textsc{public}}$ and evaluates $\textsf{public\_input}$ (zero-padded to length $2^{h_\textsc{public}}$) as a multilinear polynomial at $\vec r_\textsc{pub}$, obtaining $v_\textsc{pub} \in \Fq$. The pair $(\vec r_\textsc{pub}, v_\textsc{pub})$ is queued as an opening claim on the initial $2^{h_\textsc{public}}$-cell prefix of the committed memory polynomial, to be discharged by the WHIR step below (Lemma~\ref{lem:sz}).
...
In order to turn interractice protocol described in Section~\ref{sec:e2e-protocol} into a non-interractive one, we apply the BCS transformation \cite{BCS}. In Particular, we use the Fiat-Shamir transformation implemented via a duplex sponge  \cite{fiat_shamir_duplex_sponge} (using the $\texttt{POSEIDON}^\pi$ permutation over a state of 16 koalabear field elements).

\section{ISA programming}

This section describes \emph{conventions} for writing programs using the ISA of Section~\ref{sec:isa}. None of it is hard-coded into the VM or enforced by the snark.
...
% \paragraph{Sorting.} Sorting an array $a$ of length $n$ from scratch costs $O(n \log n)$ comparisons (see Section~\ref{sec:range-checks}). We can instead hint a candidate sorted array $b$ \emph{together with} a list of indices $\pi(0), \dots, \pi(n-1) \in [0, n)$ claiming that $b[i] = a[\pi(i)]$. The program then only has to verify
% \begin{enumerate}
%     \item $b$ is non-decreasing: one comparison per adjacent pair, $n-1$ in total.
%     \item $b[i] = a[\pi(i)]$ for every $i \in [0, n)$: $n$ memory reads.
%     \item $\pi(i) < n$ for every $i \in [0, n)$: one range check per index (Section~\ref{sec:range-checks}).
...
\paragraph{No allocation pointer.} Cairo \cite{cairo} keeps a register $\textbf{ap}$ pointing to the next free memory cell. leanVM does not: the write-once memory (Section~\ref{sec:memory}) prevents any frame from overwriting previously-written data. And since a function's internal logic should only depend on its inputs, not on where its frame happens to land, the exact value of $\textbf{fp}_\text{callee}$ does not matter to the verifier.
...
A switch dispatches to one of $k$ different code blocks based on a runtime value $x$ known to satisfy $0 \leq x < k$ (if $x$ came from a hint, it must be range-checked first; see Section~\ref{sec:range-checks}).
...
A high-level zkDSL (with mutable variables, loops, and conditionals) can be compiled to the ISA, by systematically translating mutation into \emph{Static Single-Assignment} (SSA) form, and every loop by a recursive function (Section~\ref{sec:loops}).
