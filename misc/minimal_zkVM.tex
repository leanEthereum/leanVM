...
\toprule
\textsf{be} & \textsf{start} & $\textsf{flag\_add}$ & $\textsf{flag\_mul}$ & $\textsf{flag\_eq}$ & \textsf{len} & $\textsf{idx}_A$ & $\textsf{idx}_B$ & $\textsf{idx}_R$ & $v_A$ \tiny{(5 cols)} & $v_B$ \tiny{(5 cols)} & \textsf{res} \tiny{(5 cols)} & \textsf{comp} \tiny{(5 cols)} & & $\textsf{aux}_2$ \\
\midrule
...
There are 29 columns. Extension field values are represented as 5 consecutive base field columns.
...
Virtual (non-committed) columns: $\textsf{multiplicity} = \textsf{start} \cdot (\textsf{flag\_add} + \textsf{flag\_mul} + \textsf{flag\_eq})$ and $\textsf{aux}_2 = 2 \cdot \textsf{be} + 4 \cdot \textsf{flag\_add} + 8 \cdot \textsf{flag\_mul} + 16 \cdot \textsf{flag\_eq} + 32 \cdot \textsf{len}$
...
\]
The domain separator $\textsf{aux}_2$ binds the mode and length to the bus data: since $\textsf{be}, \textsf{flag\_add}, \textsf{flag\_mul}, \textsf{flag\_eq}$ are boolean and the length is $\leq 2^{20}$ (Lemma~\ref{lem:len-bound}), the encoding is injective, so every $\textsf{aux}_2$ sent by the execution table is faithfully received here.
...
\begin{align*}
    ...
    e_\text{eq} &= v_A \cdot v_B + (1 - v_A)(1 - v_B)
\end{align*}
...
\begin{enumerate}
    \setcounter{enumi}{5}
    \item $\textsf{flag\_add} \cdot (\textsf{comp} - e_\text{add} - \textsf{comp\_tail}) = 0$ \hfill (additive ADD)
    \item $\textsf{flag\_mul} \cdot (\textsf{comp} - e_\text{mul} - \textsf{comp\_tail}) = 0$ \hfill (additive MUL)
    \item $\textsf{flag\_eq} \cdot (\textsf{comp} - e_\text{eq} \cdot (\textsf{comp\_tail} + \text{next}(\textsf{start}))) = 0$ \hfill (multiplicative EQ)
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{flag\_add} - \text{next}(\textsf{flag\_add})) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{flag\_mul} - \text{next}(\textsf{flag\_mul})) = 0$
    \item $(1 - \text{next}(\textsf{start})) \cdot (\textsf{flag\_eq} - \text{next}(\textsf{flag\_eq})) = 0$
