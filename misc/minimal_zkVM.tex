...
\[
\Pr_{\vec\beta \,\overset{\$}{\leftarrow}\, \Fq^n}\!\big[\,\hat{v}(\vec\beta) = \hat{v'}(\vec\beta)\,\big] \;\leq\; \frac{n}{|\Fq|}.
\]
\end{lemma}
\begin{proof}
Let $P = \hat{v} - \hat{v'}$. $P$ is a non-zero multilinear polynomial in $n$ variables, and it suffices to show $\Pr_{\vec\beta}[P(\vec\beta) = 0] \leq n/|\Fq|$. We induct on $n$. The case $n = 0$ is trivial. For $n \geq 1$, split
\[
P(X_1, \ldots, X_n) \;=\; X_n \cdot Q(X_1, \ldots, X_{n-1}) \;+\; R(X_1, \ldots, X_{n-1}),
\]
and write $\vec\beta = (\vec\beta_{<n}, \beta_n)$. If $Q = 0$, then $R$ is a non-zero multilinear polynomial in $n-1$ variables, and the induction hypothesis gives $\Pr[P(\vec\beta) = 0] \leq (n-1)/|\Fq| \leq n/|\Fq$. Otherwise $Q \neq 0$. By the induction hypothesis, $\Pr[Q(\vec\beta_{<n}) = 0] \leq (n-1)/|\Fq|$; and conditioned on $Q(\vec\beta_{<n}) \neq 0$, the polynomial $X_n \mapsto P(\vec\beta_{<n}, X_n)$ has degree exactly $1$, hence at most one root, so $\Pr[P(\vec\beta) = 0 \mid Q(\vec\beta_{<n}) \neq 0] \leq 1/|\Fq|$. A union bound gives $\Pr[P(\vec\beta) = 0] \leq n/|\Fq|$.
\end{proof}
...
\paragraph{Hashing tuples to a single field element.} Since $\textsf{bus\_width} = 16 = 2^4$, for some $\vec\beta = (\beta_1, \beta_2, \beta_3, \beta_4) \in \Fq^4$ sampled uniformly at random, we define:
\[
h_{\vec\beta}(\sigma) \;=\; \sum_{i=0}^{15} \sigma_i \cdot \hat{eq}\big(\vec\beta,\, [i]_{01}\big) \;=\; \hat{\sigma}(\vec\beta).
\]
By Lemma~\ref{lem:multilinear-sz}, two distinct tuples collide with probability at most $4/|\Fq|$.
